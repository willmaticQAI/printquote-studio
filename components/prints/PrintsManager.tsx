...
export function PrintsManager({
  ...
  async function deletePrint(id: string) {
    if (!window.confirm("Delete this print record?")) return;
      ...
      {prints.length === 0 ? (
        ...
        <Card glow>
          <CardHeader>
            ...
            <CardDescription>
              Review previous internal jobs, cost totals, edit them in the calculator, or delete records you no longer want to keep.
            </CardDescription>
          </CardHeader>
          <CardContent>
                ...
                <tbody className="divide-y divide-border">
                  {prints.map((print) => (
                    <tr key={print.id} className="transition-colors hover:bg-panel-soft/50">
                      ...
                      <td className="py-3.5 pr-4 text-muted">{print.quantity}</td>
                      <td className="py-3.5">
                        <div className="flex gap-2">
                          <Link
                            href={`/calculator?printId=${print.id}`}
